...
def main():
    ...
    #neigth = KNeighborsRegressor(n_neighbors=1, weights="distance")
    neigth = KNeighborsRegressor(n_neighbors=1, weights="uniform")
    neigth.fit(X, y)

    a = 0
    for i in range(len(X2)):
        prediction = int(neigth.predict(X2[i].reshape(1, -1)))
        if prediction == y2[i]:
            a += 1
